        ...
        public void run() {
            try{ //Try the following. If anything goes wrong, the error will be passed to the catch block
                ...
                //do {
                    ...
                    if (clientResponse.equalsIgnoreCase("2")){
                          //which will list all files, add to array then be sent to client.
                        message = "Server Option 2";
                        //System.out.println("FileNames: " + listAllFiles());
                        out.writeObject(listAllFiles());
                    }
                    else if (clientResponse.equalsIgnoreCase("3")){
                        //DownloadFile(); user specifies file, then server selects it, sends it across and client downloads it
                        message = "Server Option 3";
                    }
               // }while (!clientResponse.equalsIgnoreCase("4"));


                // if clientResponse / message is equal to 2 or 3 call relevent method.


                //Write out a response back to the client. This process is called Serialization or Marshalling
                //String message = "<h1>Happy Day2</h1>";
